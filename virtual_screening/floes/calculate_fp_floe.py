...
# Create Cube group
#group = CubeGroup(cubes=[calculate_fp, db_reader, get_sim_val])

# Add Groups to Workfloe
#job.add_group(group)

# Add Cubes to Floe
job.add_cubes(act_reader, index_in, db_reader, calculate_fp, concat_act, get_sim_val, ofs)

# Connect ports
act_reader.success.connect(concat_act.intake)
concat_act.success.connect(calculate_fp.intake)
calculate_fp.success.connect(db_reader.fp_input)
calculate_fp.success.connect(get_sim_val.fp_input)

index_in.success.connect(get_sim_val.baitset_input)
db_reader.success.connect(get_sim_val.intake)
